...
public class LibrarianDOA {
    ...
    public void addBook(Book book) {
        ...
        String query = "INSERT INTO book(BookID,Title, Genre, PublicationDate, Availability) VALUES (?, ?, ?, ?, ?)";
        try (Connection connection = dbConnector.connect();
             PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            statement.setInt(5, book.getAvailability(book.availability));
            statement.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
        ...
        try (Connection connection = dbConnector.connect();
            ...
            while (rs.next()) {
                ...
                if (genreName.equals(genre)) {
                    ...
                    System.out.println("Book : { Book Details:");
    ...
    public void getBookByAuthor(String author) {
        String Aquery = "SELECT AuthorID FROM Author WHERE FirstName = ?";
        String query = "SELECT * FROM book_author WHERE AuthorID = ?";
        try(Connection connection = dbConnector.connect();
        PreparedStatement Astatement = connection.prepareStatement(Aquery);
        PreparedStatement statement = connection.prepareStatement(query)){
